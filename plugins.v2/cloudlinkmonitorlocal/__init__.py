...
class CloudLinkMonitorLocal(_PluginBase):
    # 插件名称
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "cloudlinkmonitorlocal_"
    # 加载顺序
    plugin_order = 4
    # 可使用的用户级别
    auth_level = 1
    ...
    def remote_sync(self, event: Event):
        ...
        if event:
            ...
            if not event_data or event_data.get("action") != "cloud_link_sync_local":
                return
    ...
    def __link_sidecar_subtitles(self, source_file: Path, transferinfo: TransferInfo) -> None:
        ...
        subtitle_exts = {".srt", ".ass", ".ssa", ".sup", ".vtt", ".sub", ".idx"}
        if not source_file or not source_file.is_file() or not transferinfo or not transferinfo.target_item:
            return
        target_path = Path(transferinfo.target_item.path)
        target_dir = target_path.parent
        target_stem = target_path.stem
        if not target_dir.exists():
            logger.warning(f"字幕硬链接目标目录不存在：{target_dir}")
            return
        ...
        for subtitle in source_dir.iterdir():
            ...
            target_subtitle = target_dir / target_name
            if target_subtitle.exists():
                logger.info(f"字幕已存在，跳过：{target_subtitle}")
                continue
            try:
                os.link(subtitle, target_subtitle)
                linked_count += 1
                logger.info(f"字幕硬链接完成：{subtitle} -> {target_subtitle}")
