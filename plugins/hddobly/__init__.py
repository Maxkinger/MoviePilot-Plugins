...
class HdDoblyForumsSignin(_PluginBase):
    # 插件名称
    plugin_name = "杜比论坛签到"
    # 插件描述
    plugin_desc = "杜比论坛签到"
    # 插件图标
    plugin_icon = "hddobly.png"
    # 插件版本
    plugin_version = "0.1"
    # 插件作者
    plugin_author = "cas"
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._onlyonce:
            ...
            logger.info(f"杜比论坛签到服务启动，立即运行一次")
    ...
    def __signin(self):
        ...
        杜比论坛签到
        """
        res = RequestUtils(cookies=self._cookie).get_res(url="https://forums.orcinusorca.org")
        ...
        # 开始签到
        res = RequestUtils(headers=headers).post_res(url=f"https://forums.orcinusorca.org/api/users/{userId}", json=data)

        if not res or res.status_code != 200:
            logger.error("杜比论坛签到失败")
